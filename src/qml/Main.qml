...
ApplicationWindow {
    ...
    Component.onCompleted: {
        console.log("typeof backend.prime =", typeof backend.prime) // should print "function"
    }
    // ----------------- Pages -----------------
    StackLayout {
        id: stack
        ...
        SetupPageForm { id: setup }   // page 0
        RunPageForm   { id: run   }   // page 1
    }
    ...
    Dialog {
        ...
        contentItem: ColumnLayout {
            ...
            RowLayout {
                spacing: 8
                Button {
                    id: renameBtn; text: "Rename"
                    enabled: loadPresetDialog.selectedIndex >= 0
                    onClicked: {
                        var nm = presetNamesModel.get(loadPresetDialog.selectedIndex).name
                        renameDialog.oldName = nm
                        renameNameField.text = nm
                        renameDialog.open()
                    }
                }
                Button {
                    id: deleteBtn; text: "Delete"
                    enabled: loadPresetDialog.selectedIndex >= 0
                    onClicked: {
                        var nm = presetNamesModel.get(loadPresetDialog.selectedIndex).name
                        deleteDialog.targetName = nm
                        deleteDialog.open()
                    }
                }
    ...
    // Ready to Run → mirror Setup → Run and switch tab
    Connections {
        ...
        function onClicked() {
            ...
            for (var i = 0; i < sc.length && i < rc.length; ++i) {
                ...
                if (sc[i].enableCheck.checked || f > 0) {
                    ...
                    rc[i].setFlowValue.text = f.toFixed(2)
                    rc[i].lastFlow = f
                    rc[i].pumpNumber = i+1
                }
            }
    ...
    // ===== Run tab buttons (ONLY these send commands) =====
    // Start: send set_flow to all visible cards, start timer
    Connections {
        ...
        function onClicked() {
            forEachRunCard(function(card, pump) {
                ...
                card.lastFlow = f
                if (f > 0) backend.set_flow(pump, f)   // <-- underscore API
            })
    ...
    // Pause All
    Connections {
        target: run.pauseButton
        function onClicked() {
            backend.stop_all()   // <-- underscore API
            runTimer.stop()
        }
    }
    ...
    Connections {
        ...
        function onClicked() {
            forEachRunCard(function(card, pump) {
                if (card.selectCheck.checked) backend.stop(pump)
            })
        }
    }
    ...
    Connections {
        ...
        function onClicked() {
            ...
            // if you'd like auto-resume timer here:
            // if (!runTimer.running) runTimer.start()
        }
    }

    // ===== Setup tab: Prime sends immediately; flow edits are UI-only =====
    ...
    // Mirror flow edits into Run cards (no serial traffic here)
    Connections { target: setup.pump1.flowField; function onEditingFinished(){ var v=helper.num(setup.pump1.flowField.text); run.r1.lastFlow=v; run.r1.setFlowValue.text=v.toFixed(2) } function onAccepted(){ onEditingFinished() } }
    Connections { target: setup.pump2.flowField; function onEditingFinished(){ var v=helper.num(setup.pump2.flowField.text); run.r2.lastFlow=v; run.r2.setFlowValue.text=v.toFixed(2) } function onAccepted(){ onEditingFinished() } }
    Connections { target: setup.pump3.flowField; function onEditingFinished(){ var v=helper.num(setup.pump3.flowField.text); run.r3.lastFlow=v; run.r3.setFlowValue.text=v.toFixed(2) } function onAccepted(){ onEditingFinished() } }
    Connections { target: setup.pump4.flowField; function onEditingFinished(){ var v=helper.num(setup.pump4.flowField.text); run.r4.lastFlow=v; run.r4.setFlowValue.text=v.toFixed(2) } function onAccepted(){ onEditingFinished() } }
    Connections { target: setup.pump5.flowField; function onEditingFinished(){ var v=helper.num(setup.pump5.flowField.text); run.r5.lastFlow=v; run.r5.setFlowValue.text=v.toFixed(2) } function onAccepted(){ onEditingFinished() } }
    Connections { target: setup.pump6.flowField; function onEditingFinished(){ var v=helper.num(setup.pump6.flowField.text); run.r6.lastFlow=v; run.r6.setFlowValue.text=v.toFixed(2) } function onAccepted(){ onEditingFinished() } }
    Connections { target: setup.pump7.flowField; function onEditingFinished(){ var v=helper.num(setup.pump7.flowField.text); run.r7.lastFlow=v; run.r7.setFlowValue.text=v.toFixed(2) } function onAccepted(){ onEditingFinished() } }
    Connections { target: setup.pump8.flowField; function onEditingFinished(){ var v=helper.num(setup.pump8.flowField.text); run.r8.lastFlow=v; run.r8.setFlowValue.text=v.toFixed(2) } function onAccepted(){ onEditingFinished() } }
    Connections { target: setup.pump9.flowField; function onEditingFinished(){ var v=helper.num(setup.pump9.flowField.text); run.r9.lastFlow=v; run.r9.setFlowValue.text=v.toFixed(2) } function onAccepted(){ onEditingFinished() } }
}
